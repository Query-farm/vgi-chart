# vgi-chart VGI worker — build, unit-test, and SQL E2E targets.
#
# Usage:
#   make build      # fat JAR -> build/libs/vgi-chart-<ver>-all.jar
...
JAR_GLOB       = build/libs/vgi-chart-*-all.jar

.PHONY: test test-unit test-sql build clean
...
# SQL E2E: build the fat JAR, then run the .test files through DuckDB via
# haybarn-unittest with the worker as the VGI LOCATION.
test-sql: build
	...
	echo "Worker JAR: $$JAR"; \
	VGI_CHART_WORKER="java -jar $$(pwd)/$$JAR" \
