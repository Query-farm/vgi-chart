...
#
# Single image that serves the network transports of the `chart` VGI worker:
...
#
# vgi-chart is STATELESS: charts are rendered from the input relation with
# JFreeChart (headless java.awt) — no data file, no model registry — so there is
# no /data volume and no `farm.query.vgi.volumes` mount-discovery label. The
# image is just the shaded fat JAR + a tiny entrypoint.
# syntax=docker/dockerfile:1

# ---- build stage -----------------------------------------------------------
# JDK image builds the shaded fat JAR via the Gradle wrapper. The VGI Java SDK
# (farm.query:vgi / farm.query:vgirpc) and JFreeChart resolve from Maven Central,
# so the build needs only network to Central — no sibling checkout, no composite
...
RUN --mount=type=cache,target=/root/.gradle \
    ...
    && cp build/libs/vgi-chart-*-all.jar /worker.jar
...
ARG SOURCE_URL=https://github.com/Query-farm/vgi-chart
...
LABEL org.opencontainers.image.title="vgi-chart" \
      org.opencontainers.image.description="Render charts (line/bar/pie/scatter/histogram) from a query result into PNG image BLOBs via JFreeChart, as a VGI worker for DuckDB/SQL (stdio + HTTP + AF_UNIX)" \
...
ENV PORT=8000 \
    ...
    VGI_CHART_GIT_COMMIT=${GIT_COMMIT} \
    # Force headless AWT (Main also sets this) so JFreeChart never needs a display.
    ...
    JAVA_TOOL_OPTIONS="--add-opens=java.base/java.nio=ALL-UNNAMED --enable-native-access=ALL-UNNAMED -Djava.awt.headless=true"
