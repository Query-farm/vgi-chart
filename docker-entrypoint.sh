...
#
# Dispatch the single vgi-chart image into one of its transports:
...
#                    $VGI_UNIX_SOCK or /tmp/vgi-chart.sock).
...
#
# The worker is stateless (charts rendered from the input relation), so there is
# no /data to create and no state env to wire — each mode just exec's the fat JAR.
set -e
...
case "${1:-http}" in
  ...
  unix)
    shift 2>/dev/null || true
    SOCK="${1:-${VGI_UNIX_SOCK:-/tmp/vgi-chart.sock}}"
